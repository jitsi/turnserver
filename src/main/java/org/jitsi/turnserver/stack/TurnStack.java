...
public class TurnStack extends StunStack
{
	 ...
	 */
	private static final Logger logger = Logger.getLogger(TurnStack.class
	                .getName());
				...
				synchronized (serverAllocations)
				{
					...
					} else
					{
						                ...
						                .values().iterator(); i.hasNext();)
						{
							...
							{
								logger.finer("allocation " + allocation
								                + " expired");
								i.remove();
								allocation.expire();
							}
	...
	public static boolean isTurnMessage(Message message)
	{
		...
		logger.finest("method extracted from " + (int) message.getMessageType()
		                + " is : " + (int) method);
		...
		try
		{
			...
			while ((line = br.readLine()) != null)
			{
				...
				LongTermCredential ltc = new LongTermCredential(
				                tok[0].getBytes("UTF-8"),
				                tok[1].getBytes("UTF-8"));
				...
				LongTermCredentialSession ltcs = new LongTermCredentialSession(
				                ltc,
				                TurnStackProperties.DEFAULT_REALM
				                                .getBytes("UTF-8"));
