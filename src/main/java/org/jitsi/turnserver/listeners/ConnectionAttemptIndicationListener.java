...
public class ConnectionAttemptIndicationListener extends IndicationListener
{
	...
	public void handleIndication(Indication ind, Allocation alloc)
	{
		if (ind.getMessageType() == Message.CONNECTION_ATTEMPT_INDICATION)
		{
			...
			logger.finest("Received a Connection Attempt Indication with connectionId-"
			...
			try
			{
				...
				try
				{
					...
					if (evt != null)
					{
						...
						{
							logger.finest("Received a ConnectionBind Sucess Response.");
							String myMessage = "Aakash Garg";
							RawMessage rawMessage = RawMessage.build(
							                myMessage.getBytes(),
							                myMessage.length(), serverAddress,
							                localAddr);
							try
							{
								logger.finest("--------------- Thread will now sleep for 20 sec.");
								Thread.sleep(20 * 1000);
							} catch (InterruptedException e)
							{
								System.err.println("Unable to stop thread");
							}
							logger.finest(">>>>>>>>>>>> Sending a Test message : ");
						...
						{
							logger.finest("Received a ConnectionBind error Response - "
							                + msg.getAttribute(Attribute.ERROR_CODE));
						}
					} else
					{
						System.err.println("No response received to ConnectionBind request");
					}
