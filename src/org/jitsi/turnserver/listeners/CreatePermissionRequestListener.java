...
{
	/**
	 * The <tt>Logger</tt> used by the <tt>CreatePermissionRequestListener</tt> class
	 * and its instances for logging output.
	 */
	private static final Logger logger 
		= Logger.getLogger(CreatePermissionRequestListener.class.getName());
	...
	public CreatePermissionRequestListener(StunStack stunStack) 
	{
    	    ...
    	    throws IllegalArgumentException 
        {
        	...
        	if(message.getMessageType()==Message.CREATEPERMISSION_REQUEST)
        	{
        	    	...
        	    	else if(inSufficientCapacity)
        	    	{
        	    	    response = MessageFactory
        	    		    .createCreatePermissionErrorResponse(
        	    			    ErrorCodeAttribute.INSUFFICIENT_CAPACITY);
        	    	}
    	...
    	/**
	 * Starts this <tt>CreatePermissionRequestListener</tt>. If it is not currently
